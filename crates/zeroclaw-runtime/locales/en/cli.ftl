...
cli-doctor-security-audit-enabled-has-no-effect = security.audit.enabled=true has no effect: command audit logging is not wired into the runtime, so no commands are written to the configured audit path. Use an external supervisor or logging wrapper that observes the ZeroClaw process, or enable OS-level process accounting.
