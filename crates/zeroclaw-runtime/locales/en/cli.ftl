...
cli-eval-calibrate-jsonl-io = Could not access calibration JSONL {$path}: {$error}
cli-eval-calibrate-jsonl-decode = Invalid calibration JSONL {$path} at line {$line}: {$error}
cli-eval-calibrate-jsonl-encode = Could not encode calibration JSONL for {$path}: {$error}
cli-eval-calibrate-jsonl-wrong-schema = Calibration JSONL {$path} line {$line} has schema '{$found}', expected '{$expected}'.
cli-eval-calibrate-calibration-io = Could not read calibration file {$path}: {$error}
cli-eval-calibrate-calibration-malformed = Calibration file {$path} is not valid JSON: {$error}
cli-eval-calibrate-calibration-wrong-schema = Schema is '{$found}', expected '{$expected}'.
cli-eval-calibrate-calibration-wrong-judge-ref = judge_ref is '{$found}', expected '{$expected}'.
cli-eval-calibrate-calibration-insufficient-records = labeled_records is {$found}, but at least {$minimum} are required.

cli-eval-calibrate-label-records-not-found = Judge-run records file not found: {$path}
cli-eval-calibrate-label-no-records = No judge-run records found in {$path}.
cli-eval-calibrate-label-multiple-judge-refs = Records contain multiple judge refs; pass --judge-ref. Available refs: {$refs}
cli-eval-calibrate-label-judge-ref-not-found = No records found for judge ref '{$judge_ref}'. Available refs: {$refs}
cli-eval-calibrate-label-labeler-required = No labeler was provided and git config user.name is not set; pass --labeler.
cli-eval-calibrate-label-loaded = Loaded {$records} judge-run records for {$judge_ref}; {$completed} already labeled, {$pending} pending.
cli-eval-calibrate-label-nothing-pending = All {$records} judge-run records for {$judge_ref} are already labeled.
cli-eval-calibrate-label-record = Calibration record {$current} of {$total}
cli-eval-calibrate-label-task-turns = Task transcript:
cli-eval-calibrate-label-task-turn =   [{$index}] {$turn}
cli-eval-calibrate-label-transcript-hidden = Task transcript hidden. Press t to show it.
cli-eval-calibrate-label-transcript-shown = Task transcript shown.
cli-eval-calibrate-label-transcript-now-hidden = Task transcript hidden.
cli-eval-calibrate-label-rubric-name = Rubric: {$rubric}
cli-eval-calibrate-label-rubric-text = Rubric criteria: {$text}
cli-eval-calibrate-label-final-response = Final response:
cli-eval-calibrate-label-prompt = Label [p]ass / [f]ail / [u]skip / [t]oggle transcript / [q]uit:
cli-eval-calibrate-label-invalid-choice = Enter p, f, u, t, or q.
cli-eval-calibrate-label-skipped = Skipped; this record remains pending.
cli-eval-calibrate-label-judge-pass = pass
cli-eval-calibrate-label-judge-fail = fail
cli-eval-calibrate-label-human-pass = pass
cli-eval-calibrate-label-human-fail = fail
cli-eval-calibrate-label-reveal = Judge verdict: {$verdict} (score {$score}). Reason: {$reason}
cli-eval-calibrate-label-saved = Saved human {$verdict} label to {$path}.
cli-eval-calibrate-label-quit = Labeling stopped: {$written} written this run, {$pending} still pending.
cli-eval-calibrate-label-complete = Labeling complete: {$written} written this run, {$pending} still pending.

cli-eval-calibrate-finalize-no-labels = No judge labels found in {$path}.
cli-eval-calibrate-finalize-multiple-judge-refs = Labels must share one judge ref; found: {$refs}
cli-eval-calibrate-finalize-too-few = Need at least 50 labeled records; found {$count} ({$remaining} more needed).
cli-eval-calibrate-finalize-invalid-min-agreement = --min-agreement must be between 0 and 1 inclusive; got {$value}.
cli-eval-calibrate-finalize-summary = Overall agreement: {$agreement}% ({$agreed}/{$total})
cli-eval-calibrate-finalize-rubrics = Per-rubric agreement:
cli-eval-calibrate-finalize-rubric =   {$rubric}: {$agreement}% ({$agreed}/{$total})
cli-eval-calibrate-finalize-kappa = Cohen's kappa: {$kappa}
cli-eval-calibrate-finalize-kappa-undefined = Cohen's kappa: undefined (both raters assigned every label to the same class)
cli-eval-calibrate-finalize-low-agreement = Warning: agreement {$agreement}% is below the recommended 85% calibration floor; emitting the calibration file.
cli-eval-calibrate-finalize-min-agreement-refused = Refusing to emit: agreement {$agreement}% is below --min-agreement {$minimum}%.
