...
Pre-attribution rows have no owner and fail closed. Startup assigns them automatically when exactly one agent is enabled. In a multi-agent install, the tool remains unavailable until `knowledge.legacy_owner_agent` names an enabled agent. The runtime warns with the alias and assigned row count; remove the key after the initial migration so later unowned imports or rollback-written rows are not silently assigned to that alias. After assignment, normal allowlists control any sharing.

## Rename and delete lifecycle

Use the gateway dashboard's agent controls or the dedicated `zeroclaw agents` CLI for rename and delete. In the standard build with `gateway` and `agent-runtime` enabled, both surfaces run the initial reference and owned-state cascades; directly removing or re-keying `agents.<alias>` in TOML or through a generic config setter does not. A reduced-feature CLI still updates config references but warns that owned state was not cascaded, so use a build with both features enabled for lifecycle operations.
...
The post-persist side effects are best-effort and report surfaced failures, but archive-file write failures may appear only in gateway logs. Delete retries use the shared residue resolver. Committed-rename recovery is narrower: the gateway API can retry residue left under the old alias through a gateway-local probe, and the RPC lifecycle path can resume the shared owned-state cascade when the destination exists and residue remains. The CLI cannot yet retry after the source alias has been durably removed. [Issue #10373](https://github.com/zeroclaw-labs/zeroclaw/issues/10373) tracks one shared committed-rename recovery contract across these surfaces. After deletion, verify the archive contents and logs before relying on the archive for recovery. Automated restore is not supported.
