...
│   │   ├── *.jsonl             # explicit legacy chat/session backend
│   │   ├── *.metadata.json     # JSONL ownership and routing metadata
...
| Chat and channel sessions | `[channels].session_backend` plus `SessionBackend` | Default `data/sessions/sessions.db`; legacy/explicit JSONL uses `data/sessions/*.jsonl` plus `*.metadata.json` ownership sidecars | `zeroclaw-infra` backend handles are currently constructed independently by channels, gateway, RPC, and session tools | SQLite backend uses WAL; `SessionActorQueue` serializes active turns per session; JSONL message mutations and metadata replacement share one process-local per-directory sessions lock | Chat/Code sessions use the unified backend contract. Legacy JSONL files without a sidecar remain readable but unattributed, so agent-scoped session tools fail closed until a trusted gateway or channel path stamps ownership. A JSONL ownership claim materializes an empty transcript, and retention keeps unmessaged claims (including legacy metadata-only claims) until their first message so attribution cannot detach. Channel rows also carry a trusted channel marker; channel startup ignores agent-attributed gateway/RPC rows in the same backend. Switching to SQLite imports the transcript and sidecar together. ACP protocol sessions use a separate store. Process-level backend ownership is not yet single-sourced. |
